...
std::map<int, fluid_player_t *> players = {}; // MIDI player (optional, per sfId)
int nextSfId = 1;
...
Java_com_melihhakanpektas_flutter_1midi_1pro_FlutterMidiProPlugin_dispose(JNIEnv *env, jclass clazz)
{
    ...
    {
        fluid_player_stop(x.second);
        delete_fluid_player(x.second);
    }
    players.clear();
...
// ─── MIDI Player (fluid_player_t) ───────────────────────────────────────────

extern "C" JNIEXPORT void JNICALL
Java_com_melihhakanpektas_flutter_1midi_1pro_FlutterMidiProPlugin_playMidiBuffer(JNIEnv *env, jclass clazz, jint sfId, jbyteArray midiData)
{
    // Stop any existing player for this sfId
    if (players.find(sfId) != players.end())
    {
        fluid_player_stop(players[sfId]);
        delete_fluid_player(players[sfId]);
    ...
    fluid_player_t *player = new_fluid_player(synths[sfId]);
    if (!player)
        return;

    jsize len = env->GetArrayLength(midiData);
    jbyte *data = env->GetByteArrayElements(midiData, nullptr);
    // fluid_player_add_mem expects a pointer and length
    fluid_player_add_mem(player, (const char *)data, (size_t)len);
    env->ReleaseByteArrayElements(midiData, data, JNI_ABORT);
...
extern "C" JNIEXPORT void JNICALL
Java_com_melihhakanpektas_flutter_1midi_1pro_FlutterMidiProPlugin_stopMidiPlayer(JNIEnv *env, jclass clazz, jint sfId)
{
    if (players.find(sfId) == players.end())
        return;
    fluid_player_stop(players[sfId]);
